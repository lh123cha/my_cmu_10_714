...
def xavier_uniform(fan_in, fan_out, gain=1.0, **kwargs):
    ...
    return rand(shape,low=-a,high=a).reshape((fan_in,fan_out))
    ### END YOUR SOLUTION
...
def xavier_normal(fan_in, fan_out, gain=1.0, **kwargs):
    ...
    shape = fan_in*fan_out
    return randn(shape,mean=0,std=std).reshape((fan_in,fan_out))
    ### END YOUR SOLUTION
...
def kaiming_uniform(fan_in, fan_out, nonlinearity="relu", **kwargs):
    ...
    return rand(shape,low=-bound,high=bound).reshape((fan_in,fan_out))
    ### END YOUR SOLUTION
